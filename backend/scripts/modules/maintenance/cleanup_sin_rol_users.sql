-- Script SQL para eliminar completamente usuarios con rol 'sin_rol'
-- Ejecutar con cuidado, esto eliminará permanentemente los datos

-- Primero identificamos los usuarios problemáticos
DO $$
DECLARE
    user_record RECORD;
...
BEGIN
    ...
    FOR user_record IN 
        ...
        WHERE ur.role_id = (SELECT id FROM roles WHERE name = 'sin_rol')
        AND ur.is_active = true
        AND u.id NOT IN (
            SELECT DISTINCT ur2.user_id 
            FROM user_roles ur2 
            WHERE ur2.role_id != (SELECT id FROM roles WHERE name = 'sin_rol')
    ...
    LOOP
        ...
        RAISE NOTICE 'Usuario % eliminado exitosamente', user_record.email;
...
-- Verificar que no quedan usuarios con rol 'sin_rol'
SELECT 
    u.email,
    u.first_name,
    u.last_name,
...
HAVING COUNT(ur.id) = (
    SELECT COUNT(*) 
    FROM user_roles ur2 
    WHERE ur2.user_id = u.id AND ur2.is_active = true
); 